...
import com.rn.apigateway.config.RoutingConfig;
...
@Component
public class JwtSignInFilter extends SignInFilter {
        ...
        return chain.filter(exchange).then(Mono.fromRunnable(() -> {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                refreshTokenRepository
                    .deleteById(authenticationBody.getId())
                    .then(refreshTokenRepository.save(authenticationBody.getId(), refreshToken));
            }
        }));
    }
}
